		...
		if(req.method === 'GET') {
			if(module ==="productos") {
				if(!!titulo || !!categoria) {
					...
					if(!!categoria) {
						filterProductos = entitiesData.filter(p => p.categoria.toLowerCase().includes(categoria))
					}

					if(!!titulo) {
						filterProductos = filterProductos.filter(p => p.titulo.toLowerCase().includes(titulo.toLowerCase()))
					}
				...
				if(!!titulo) {
					...
					filterTitulo = entitiesData.filter(c => c.titulo.toLowerCase().includes(categoriaTitulo))
				...
				if(!!usuario) {
					...
					filterUsuario = entitiesData.filter(u => {
			...
			const { id } = req.body;
			if (entitiesData.some(data => data.id === id)) {
				res.status(400).end();
			} else {
